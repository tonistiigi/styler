...
(function() {
  var fs, mkdirp, os, path, request, winston, _;
  ...
  exports.getFileLocation = function(project, fileurl, cb) {
    ...
    if (files) {
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        ...
        if (0 === fileurl.indexOf(url)) {
          ...
          return path.exists(srcfile, function(found) {
            if (found) {
              return cb(null, srcfile, cssfile);
            } else if (newfiles && fileurl.match(/^http/)) {
              winston.notice('File was not found. Trying to create new.', {
                project: project.id,
                file: srcfile
              });
              return request(fileurl, function(error, response, body) {
                ...
                if (error || response.statusCode !== 200) {
                  winston.warning('Failed to get contents for the file.', {
                    file: srcfile,
                    url: fileurl,
                    err: error,
                  ...
                  winston.info('Received contents for file.', {
                    file: srcfile,
                    url: fileurl,
                ...
                writefile = function() {
                  return fs.writeFile(srcfile, body, 'utf8', function(err) {
                    if (err) {
                      winston.error('Failed to write new file', {
                        file: srcfile,
                        err: err
                      });
                      return cb(true);
                    } else {
                      winston.info('Created new file', {
                        file: srcfile
                      });
                      return cb(null, srcfile, cssfile);
                    }
                ...
                return path.exists(srcdir, function(found) {
                  if (found) {
                    return writefile();
                  } else {
                    winston.info('Creating directory', {
                      path: dir
                    });
              ...
              winston.notice('File was not found. Igroing.', {
                file: srcfile
              });
              return cb(true);
            }
    ...
    winston.debug('File didn\'t match project configuration', {
      project: project.id,
      url: fileurl
    });
    return cb(true);
  ...
  */
  ...
  exports.getDriveNames = function(cb) {
    ...
    return exec(command, function(error, stdout, sterr) {
      var match, matches;
      if (error) {
        winston.warning('Failed to get drive names using wmic', {
          err: error,
          output: stdout
        });
