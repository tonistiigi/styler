...
(function() {
  ...
  _ref = require("path"), existsSync = _ref.existsSync, join = _ref.join;
  ...
  Backbone.serverSync = function(method, name, model, options, success) {
    ...
    return Backbone.connector.backends[name][action](model, options, function(data) {
      if (method === "read") {
        ...
        return io.server.of('/console')["in"]('backbone').emit('BackboneSync', name, method, data, options);
      }
    });
  };
