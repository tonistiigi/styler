...
(function() {
  var Clients, Projects, Settings, fs, getKeyCommands, io, isAllowedIP, useragent, winston, _, _ref;
  ...
  exports.callClient = function(clientId, method, params, cb) {
    ...
    if (method === 'focus') {
      ...
      return io.server.of('/info').emit('focus', id);
    } else {
  ...
  io.server.of("/info").on('connection', function(socket) {
  ...
  io.server.of("/clients").on('connection', function(socket) {
    ...
    onClientRegistred = function(_client) {
      ...
      return winston.info('Client connected', {
        id: client.id,
        project: projectId,
        url: client.get('url')
      });
    ...
    return socket.on("disconnect", function() {
      ...
      return winston.debug('Client disconnected', {
        id: client.id
      });
    });
  });
